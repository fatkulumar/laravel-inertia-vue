...
watch(search, (value) => {
  router.get(
    `/dashboard/regency-regional/${props.regional_id}`,
    { search: value },
    {
      preserveState: true,
      replace: true,
...
function modalRegencyRegional(opt) {
...
function addRegencyRegional() {
  form.post("/dashboard/regency-regional/store", {
    preserveScroll: true,
    onSuccess: () => {
      resetForm();
      modalRegencyRegional("hide");
...
function editRegencyRegional(data) {
  form.id = data.id;
  form.regency = data.regency;
  modalRegencyRegional("show");
}

function deleteRegencyRegional(id, name) {
  const konfirm = confirm(`Apakah anda yakin ingin menghapus ${name}?`);
  if (!konfirm) return;
  form.delete(`/dashboard/regency-regional/delete/${id}`, {
    preserveScroll: true,
    onSuccess: (e) => {
      toast("success", "Berhasil");
      resetForm();
...
function toggleCheckbox(id) {
  ...
  if (props.regencyRegionals.to == totalChecked) {
...
function checkedAll() {
  ...
  if (checboxAll.checked == true) {
    ...
    props.regencyRegionals.data.forEach((data) => {
...
function deleteRegencyRegionalDestroy() {
  const konfirm = confirm(`Apakah anda yakin ingin menghapus data ini?`);
  if (!konfirm) return;
  formCheckbox.post("/dashboard/regency-regional/destroy", {
    preserveScroll: true,
    onSuccess: () => {
      formCheckbox.id = [];
    ...
    <AuthenticatedLayoutAdmin>
      <template #header>
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
          Data MWC <span class="underline"> {{ props.regencyRegionals.data[0]?.regional?.name }} </span>
        </h2>
      </template>
      <div class="py-12">
            ...
            <div class="p-6 text-gray-900">
              <div class="overflow-x-auto shadow-md sm:rounded-lg">
                ...
                >
                    ...
                    <tr>
                      ...
                      <th scope="col" class="px-6 py-3">
                        <div class="flex gap-1">
                          ...
                          />
                          <div
                            @click="deleteRegencyRegionalDestroy()"
                            v-show="deleteChoice"
                            title="Hapus"
                  ...
                  <tbody>
                    <tr
                      v-for="(item, index) in props.regencyRegionals.data"
                      :key="index"
                      class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                    >
                      <td class="px-6 py-4">
                        {{ props.regencyRegionals.from + index }}
                      </td>
                      <td class="px-6 py-4">
                        {{ item.regency }}
                      </td>
                        ...
                        <div class="flex gap-2">
                          ...
                          >
                            ...
                            <li>
                                <button
                                  title="Edit"
                                  @click="editRegencyRegional(item)"
                                  type="button"
                              ...
                              <li>
                                <button
                                  title="Hapus"
                                  @click="deleteRegencyRegional(item.id, item.title)"
                                  type="button"
                                  class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                >
                                  Hapus
              ...
              <Pagination
                ...
                :links="props.regencyRegionals.links"
              />
            </div>
          </div>
        </div>
      ...
      >
        <div class="relative p-4 w-full max-w-7xl max-h-full">
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <form @submit.prevent="addRegencyRegional" class="p-4 md:p-5">
