...
function goToDataMwc(id) {
  window.location.href = `/dashboard/regency-regional/${id}`;
}
...
function toggleCheckbox(id) {
  ...
  if (props.regionals.to == totalChecked) {
...
function checkedAll() {
  ...
  if (checboxAll.checked == true) {
    ...
    props.regionals.data.forEach((data) => {
...
function deleteRegionalChoice() {
  ...
  if (!konfirm) return;
  formCheckbox.post("/dashboard/regional/destroy", {
    preserveScroll: true,
    onSuccess: () => {
      formCheckbox.id = [];
      toast("success", "Data Berhasil Dihapus");
            ...
            <div class="p-6 text-gray-900">
              <div class="overflow-x-auto shadow-md sm:rounded-lg">
                ...
                >
                  ...
                  <tbody>
                    <tr
                      v-for="(item, index) in props.regionals.data"
                      :key="index"
